...
from AllCartsExt import AllCartsExt
from ShopifyCartGetsExt import ShopifyCartGetsExt
from ShopifyCartCustomsExt import ShopifyCartCustomsExt
...
class ShopifyCart(
    AllCartsExt, ShopifyCartGetsExt, ShopifyCartCustomsExt
):
    def __init__(self, _company_name, _printing=False):
        self.company_name = _company_name
        self.printing = _printing
        self.settings = self.getSettings()
        self.shop = self.openShop()



    def getSettings(self):
        """
        output: Return settings_, a module containing designated data for processing company's
                Shopify orders.
        """
        ...
        format_largs = [os.getcwd(), self.company_name]
        file_path = '{}/shops_settings/{}_shopify_settings.py'.format(*format_largs)
        file_name = '{}_shopify_settings'.format(self.company_name)
        spec = importlib.util.spec_from_file_location(file_name, file_path)
        settings_ = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(settings_)
        return settings_
    ...
    def openShop(self):
        """
        input:  _print = Bool determining whether actions are printed to console (for debug).
        ...
        shop_creds = { key: self.settings.credentials[key] for key in creds_dict_keys }
        ...
        if self.printing:
            print(">>> you now have API access to the *{}* Shopify shop".format(shop_.name))
        return shop_



    def getAllShopProducts(self, _batch_size=250):
        """
    ...
    def getShopProductDetails(self, _product):
        """
        input:  _product = A shopify product object.
        output: Return details_, a list of product details of each variant of product.  Many
                products have only a single variant, so return a list of a single product's details.
    ...
    def prettyPrintShopProducts(self, _products):
        """
    ...
    def prettyPrintShopOrder(self, _shop_order):
